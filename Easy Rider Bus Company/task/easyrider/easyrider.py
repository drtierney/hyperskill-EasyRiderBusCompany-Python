...
for entry in data:
    ...
    if type(entry['stop_name']) != str or not re.match(stop_name_regex, entry['stop_name']):
    ...
    if type(entry['a_time']) != str or not re.match(a_time_regex, entry['a_time']):
        error_counter['a_time'] += 1

error_count = sum(error_counter.values())
print(f"Format validations: {error_count} errors")
